...
`0.1.8` — ранняя версия. Индексация, импорт артефактов, поиск, карта и пробный рост не переписывают тела исходных заметок. Низкоуровневые инструменты и внутренние границы модулей еще могут меняться, пока сервер полируется.
