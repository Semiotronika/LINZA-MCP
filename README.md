...
`0.1.5` — ранняя версия. Индексация, импорт артефактов, поиск, карта и пробный рост не переписывают тела исходных заметок. Низкоуровневые инструменты и внутренние границы модулей еще могут меняться, пока сервер полируется.
