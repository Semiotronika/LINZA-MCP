...
Внутри каждое предложение для ревью остается структурой с номером, доказательствами и пробным применением. Наружу LINZA отдает готовые строки для пользователя, чтобы агент мог не пересказывать JSON, а просто написать нормальный ответ.
...
Это минимальная конфигурация для старта. LINZA может запуститься и показать
инструменты без embedding-сервера. Переменные эмбеддингов добавляйте для
смыслового поиска, карты тем и предложений связей:
...
LINZA проектируется как локальный служебный слой с проверкой пользователем:
...
`0.1.9` — ранняя версия. Индексация, импорт артефактов, поиск, карта и пробный рост не переписывают тела исходных заметок. Низкоуровневые инструменты и внутренние границы модулей еще могут меняться, пока сервер полируется.
...
| Переменная | Обязательна для старта? | Описание |
|---|---:|---|
| `LINZA_VAULT` | Нет | Путь к Markdown-папке; по умолчанию `./vault` |
| `LINZA_EMBED_PROVIDER` | Нет | `lmstudio` для нормальной локальной работы; также поддерживаются `openai`, `ollama` |
| `LINZA_EMBED_URL` | Нет | URL embeddings API; по умолчанию `http://127.0.0.1:1234/v1` |
| `LINZA_EMBED_MODEL` | Нет | Модель для эмбеддингов; задайте перед смысловой индексацией/поиском |
...
| `LINZA_TOOL_SURFACE` | Нет | `default` (7 инструментов) или `advanced` |
| `LINZA_LANGUAGE` | Нет | Язык подсказок, статуса и ревью-маршрута в `guide_next_steps`: `auto`, `ru`, `en` |
