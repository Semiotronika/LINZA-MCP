...
| `smoke_copy_vault.py` | Run a private-output-safe public-facade workflow smoke test against a copied vault. |
...
`smoke_copy_vault.py` defaults to deterministic local smoke embeddings so it can
exercise `guide_next_steps` and `agent_workspace` without requiring LM Studio.
Pass `--embed-provider lmstudio` when you want to test a real local embeddings
server.
